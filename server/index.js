...
// Serve static from /public (robust: works even if Render Root Directory is set to src)
...
const PUBLIC_DIR =
  PUBLIC_DIR_CANDIDATES.find(p => fs.existsSync(path.join(p, 'index.html'))) ||
  PUBLIC_DIR_CANDIDATES.find(p => fs.existsSync(p)) ||
  null;

if (PUBLIC_DIR) {
  app.use(express.static(PUBLIC_DIR));

  // SPA fallback (avoid "Cannot GET /" on refresh/deep links)
  ...
  console.warn('[WARN] public/ folder not found. Frontend will not be served by this service.');
}
